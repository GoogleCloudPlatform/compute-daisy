...
func newTestGCSClient() (*storage.Client, error) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if match := uploadRgx.FindStringSubmatch(u); m == "POST" && match != nil {
		...
		} else if m == "GET" && u == "/b/gcs/o?alt=json&delimiter=&endOffset=&includeTrailingDelimiter=false&matchGlob=&pageToken=&prefix=folder%2F&prettyPrint=false&projection=full&startOffset=&versions=false" {
			fmt.Fprint(w, `{}`)
		} else if m == "GET" && u == "/b/bucket/o?alt=json&delimiter=&endOffset=&includeTrailingDelimiter=false&matchGlob=&pageToken=&prefix=object%2F&prettyPrint=false&projection=full&startOffset=&versions=false" {
			fmt.Fprint(w, `{}`)
			...
			fmt.Fprint(w, `{}`)
		} else if m == "GET" && u == "/b/foo/o?alt=json&delimiter=&endOffset=&includeTrailingDelimiter=false&matchGlob=&pageToken=&prefix=bar&prettyPrint=false&projection=full&startOffset=&versions=false" {
			fmt.Fprint(w, `{}`)
