...
func TestSubnetworkValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "explicit-ipv4-success",
			sn:        &Subnetwork{Subnetwork: compute.Subnetwork{Name: "foo", Network: "bar", StackType: "IPV4_ONLY", IpCidrRange: "192.168.1.0/32"}},
			shouldErr: false,
		},
		{
			name:      "explicit-ipv4-failure",
			sn:        &Subnetwork{Subnetwork: compute.Subnetwork{Name: "foo", Network: "bar", StackType: "IPV4_ONLY", IpCidrRange: "192.168.1.0/33"}},
			shouldErr: true,
		},
		{
			name:      "ipv4-ipv6-success",
			sn:        &Subnetwork{Subnetwork: compute.Subnetwork{Name: "foo", Network: "bar", StackType: "IPV4_IPV6", IpCidrRange: "192.168.1.0/32", Ipv6AccessType: "EXTERNAL"}},
			shouldErr: false,
		},
		{
			name:      "ipv4-ipv6-failure",
			sn:        &Subnetwork{Subnetwork: compute.Subnetwork{Name: "foo", Network: "bar", StackType: "IPV4_IPV6", IpCidrRange: "192.168.1.0/33"}},
			shouldErr: true,
		},
		{
			name:      "ipv6-success",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.shouldErr && err == nil {
				t.Errorf("validate(ctx, s) should have returned an error but didn't")
